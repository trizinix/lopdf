...
fn escape_sequence() -> Parser<u8, Vec<u8>> {
	sym(b'\\')
		* (sym(b'\\').map(|_| vec![b'\\']) | sym(b'(').map(|_| vec![b'(']) | sym(b')').map(|_| vec![b')']) | sym(b'n').map(|_| vec![b'\n']) | sym(b'r').map(|_| vec![b'\r'])
			| sym(b't').map(|_| vec![b'\t']) | sym(b'b').map(|_| vec![b'\x08']) | sym(b'f').map(|_| vec![b'\x0C']) | oct_char().map(|c| vec![c]) | eol().map(|_| vec![]) | empty().map(|_| vec![]))
}
...
fn object_id() -> Parser<u8, ObjectId> {
	let id = one_of(b"0123456789")
		.repeat(1..)
		.convert(|v| u32::from_str(&String::from_utf8(v).unwrap()));
	let gen = one_of(b"0123456789")
		.repeat(1..)
		.convert(|v| u16::from_str(&String::from_utf8(v).unwrap()));
	id - space() + gen - space()
}

pub fn direct_object() -> Parser<u8, Object> {
	(seq(b"null").map(|_| Object::Null) | seq(b"true").map(|_| Object::Boolean(true)) | seq(b"false").map(|_| Object::Boolean(false)) | object_id().map(|id| Object::Reference(id)) - sym(b'R')
		| real().map(|num| Object::Real(num)) | integer().map(|num| Object::Integer(num)) | name().map(|bytes| Object::Name(bytes)) | literal_string().map(Object::string_literal)
		| hexadecimal_string().map(|bytes| Object::String(bytes, StringFormat::Hexadecimal)) | array().map(|items| Object::Array(items)) | dictionary().map(|dict| Object::Dictionary(dict))) - space()
}

fn object<'a>(reader: &'a Reader) -> parser::Parser<'a, u8, Object> {
	(seq(b"null").map(|_| Object::Null) | seq(b"true").map(|_| Object::Boolean(true)) | seq(b"false").map(|_| Object::Boolean(false)) | object_id().map(|id| Object::Reference(id)) - sym(b'R')
		| real().map(|num| Object::Real(num)) | integer().map(|num| Object::Integer(num)) | name().map(|text| Object::Name(text)) | literal_string().map(Object::string_literal)
		| hexadecimal_string().map(|bytes| Object::String(bytes, StringFormat::Hexadecimal)) | array().map(|items| Object::Array(items)) | stream(reader).map(|stream| Object::Stream(stream))
...
pub fn header() -> Parser<u8, String> {
	seq(b"%PDF-")
		* none_of(b"\r\n")
			.repeat(0..)
			.convert(|v| String::from_utf8(v)) - eol() - comment().repeat(0..)
}
...
fn operand() -> Parser<u8, Object> {
	(seq(b"null").map(|_| Object::Null) | seq(b"true").map(|_| Object::Boolean(true)) | seq(b"false").map(|_| Object::Boolean(false)) | real().map(|num| Object::Real(num))
		| integer().map(|num| Object::Integer(num)) | name().map(|text| Object::Name(text)) | literal_string().map(Object::string_literal)
		| hexadecimal_string().map(|bytes| Object::String(bytes, StringFormat::Hexadecimal)) | array().map(|items| Object::Array(items)) | dictionary().map(|dict| Object::Dictionary(dict)))
		- content_space()
}
...
pub fn content() -> Parser<u8, Content> {
	content_space()
		* operation()
			.repeat(0..)
			.map(|operations| Content { operations })
}
